...
while i <= 199:
    s = requests.Session()
    headers = {'Referer': 'https://results.vtu.ac.in/VTUResultsDecJan19/index.php',
                   'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/71.0.3578.98 Safari/537.36',
                   'Upgrade-Insecure-Requests': '1',  'Cookie': 'PHPSESSID=e0e6cf142e7c180354e2d78d0ec2eb63'
            , 'Connection': 'keep-alive'}
    image = s.get("https://results.vtu.ac.in/VTUResultsDecJan19/resultsvitavicbcs_19/captcha_new.php", headers=headers,verify=False)
    with open("snap.png", 'wb') as file:
        file.write(image.content)
    cap = ocr.get_ocr("snap.png")
    USN = "1BI18CS"+str(format(i, '03d'))
    url = "https://results.vtu.ac.in/VTUResultsDecJan19/resultsvitavicbcs_19/resultpage.php"
    payload = {'lns': USN, 'captchacode': str(cap),
                   'token': 'dWFnOFRKWndWNkd5RERKaTVoeXJ0ZDRXUTRBdThFUE1rN2ZKb0VoVER6QU0rL3NZYStEdC9iMmhNM1E0aTdyanQwN2lQdkxYeERkZHNNcEVwRjgvNnc9PTo65A1z+WC47fyEqPiw174WAA==',
                   'current_url': 'https://results.vtu.ac.in/VTUResultsDecJan19/resultsvitavicbcs_19/index.php'}
    ...
    name.clear()
    # print(page.text)
    while (page.text[temp + 62] != "<"):
        name.insert(len(name), page.text[temp + 62])
        temp += 1
    # print(name)
    # exit(3)
    names.insert(len(names), ''.join(name))
    # if("Semester : 1" in page.text):
    print("foo")
